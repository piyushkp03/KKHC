...
def buydirect():
    ...
    x=orders(name=usname,email=cartemail,address=addr,phone=ph,pid='OrID'+str(oid),pname=prname,pprice=prprice)
    x.save()
def buyfromcart():
    datas=usercart.objects.filter(email=cartemail)
    for x in datas:
        y=orders(name=usname,email=cartemail,address=addr,phone=ph,pid='OrID'+str(oid),pname=x.pname,pprice=x.pprice)
        y.save()
    datas.delete()
